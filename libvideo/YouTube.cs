...
namespace VideoLibrary
{
    public class YouTube : ServiceBase<YouTubeVideo>
    {
        ...
        private static YouTube _default;
        public static YouTube Default
        {
            get
            {
                if (_default == null)
                    _default = new YouTube();

                return _default;
            }
        }

        public ArgumentException SetDecryptionRegex(string pattern)
        {
            try
            {
                YouTubeVideo.DecryptionFunctionRegex = new System.Text.RegularExpressions.Regex(pattern);
